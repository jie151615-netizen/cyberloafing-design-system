...
(function () {
  ...
  const STRINGS = {
    zh: {
      levelName: "无限接水", levelNameEn: "Water Wanderer",
      ...
      interrupt: {
        colleague: { title: "同事走过来了！", body: "点【假装聊工作】，稳住别慌！" },
        boss: { title: "老板的余光扫过来了！", body: "点【假装看打印机】，稳住别慌！" },
      },
      chatButton: "假装聊工作",
    ...
    en: {
      ...
      interrupt: {
        colleague: { title: "A COLLEAGUE IS COMING!", body: "Hit CHAT ABOUT WORK before it's too late!" },
        boss: { title: "THE BOSS IS GLANCING OVER!", body: "Hit CHECK THE PRINTER before it's too late!" },
      },
    ...
    it: {
      ...
      interrupt: {
        colleague: { title: "STA ARRIVANDO UN COLLEGA!", body: "Premi PARLA DI LAVORO prima che sia tardi!" },
        boss: { title: "IL CAPO STA GUARDANDO!", body: "Premi CONTROLLA LA STAMPANTE prima che sia tardi!" },
      },
  ...
  const TITLES = {
    low: { zh: "茶水间新手", en: "Water-Cooler Newbie", it: "Novizio della Fontanella" },
    mid: { zh: "饮水机守护神", en: "Water Cooler God", it: "Dio della Fontanella" },
    high: { zh: "接水传奇 · 永动摸鱼机", en: "Legendary Refill Machine", it: "Macchina Leggendaria del Fannullone" },
  };
  ...
  function CoolerRig({ fillPct }) {
    const cupTop = 140, cupBottom = 230, cupH = cupBottom - cupTop;
    const waterH = (fillPct / 100) * cupH;
    return (
      <svg viewBox="0 0 200 240" width="100%" style={{ overflow: "visible", display: "block" }}>
        <defs>
          <clipPath id="cupClip"><path d="M70,140 L130,140 L120,230 L80,230 Z" /></clipPath>
        </defs>
        <ellipse cx="100" cy="38" rx="26" ry="32" fill="var(--mint-wash)" stroke="var(--ink)" strokeWidth="2.5" />
        <rect x="76" y="66" width="48" height="58" rx="8" fill="var(--paper-warm)" stroke="var(--ink)" strokeWidth="2.5" />
        <rect x="93" y="122" width="14" height="10" fill="var(--paper)" stroke="var(--ink)" strokeWidth="2" />
        <g clipPath="url(#cupClip)">
          <rect x="70" y={cupBottom - waterH} width="60" height={waterH} fill="var(--mint)" />
        </g>
        <path d="M70,140 L130,140 L120,230 L80,230 Z" fill="none" stroke="var(--ink)" strokeWidth="2.5" strokeLinejoin="round" />
      </svg>
    );
  }
  ...
  function GameCore({ lang, initialScore = 0, onScoreChange }) {
    ...
    useEffect(() => { onScoreChange && onScoreChange(seconds); }, [seconds]);
      ...
      <div className="game-core">
        <div className="hud">
          <div className="hud-block">
            ...
            <div className="hud-value">{secondsRounded.toLocaleString(lang === "zh" ? "zh-CN" : lang === "it" ? "it-IT" : "en-US")}{t.unitSuffix}</div>
            <div className="hud-sub">{t.pointsBanked}</div>
          </div>
          <div className="hud-block">
            <div className="hud-label">{t.fillLabel}</div>
            <div className="speed-bar">
              <div className="speed-fill" style={{ transform: `scaleX(${fillPct / 100})`, background: "var(--mint)" }} />
            </div>
            <div className="hud-sub">{Math.round(fillPct)}%</div>
          </div>
        </div>
        ...
        <div className="roll-stage">
          <div className="roll" style={{ cursor: "default" }}>
            <CoolerRig fillPct={fillPct} />
            {floaters.map((f) => (
              <span key={f.id} className="floater" style={{ left: `calc(50% + ${f.x}px)` }}>{t.freshCupBonus}</span>
            ))}
        ...
        <div className="alert-zone" aria-live="assertive">
          {interrupt && (
            <div className="warning-banner" style={{ background: "var(--register-light)", color: "var(--ink)" }}>
              <div className="warning-title" style={{ color: "var(--ink)" }}>{t.interrupt[interrupt.type].title}</div>
              <div className="warning-body">{t.interrupt[interrupt.type].body}</div>
              <div className="warning-bar" style={{ background: "rgba(19,49,79,.18)" }}>
                <div className="warning-bar-fill" style={{ width: interrupt.pct + "%", background: "var(--ink)" }} />
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flush-row">
          {interrupt ? (
            <div className="response-row">
              <button className="response-btn" onClick={() => resolveInterrupt("colleague")}>{t.chatButton}</button>
              <button className="response-btn" onClick={() => resolveInterrupt("boss")}>{t.printerButton}</button>
            </div>
          ) : (
            <button
              className="flush-btn armed"
              style={{ background: "var(--mint)", borderColor: "var(--mint)", color: "var(--ink)" }}
